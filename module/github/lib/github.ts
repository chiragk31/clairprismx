  ...
  for (const item of data) {
    if (item.type === "file") {
      ...
      if (
        ...
        fileData.content &&
        !item.path.match(
          /\.(jpg|jpeg|png|gif|bmp|ico|pdf|zip|tar\.gz|exe|dll)$/i,
        )
      ) {
        files.push({
          path: item.path,
...
export async function postReviewComment(
  ...
  await octokit.rest.pulls.createReview({
    ...
    repo,
    pull_number: prNumber,
    body: `##🍿ClairPrismX Code Review dev by chiragk31 𓂃✍︎\n\n${review}\n\n----\n*📌This review was generated by ClairPrismX's AI engine powered by gemini and developed by chiragk31🤓.*`,
  });
}
