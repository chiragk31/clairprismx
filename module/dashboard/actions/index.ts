...
//         const totalRepos = 30;
//         const calendarData = await fetchUserContribution(user.login, token);

//         const totalCommits = calendarData?.totalContributions || 0;
//         //counting the pr from database or github
...
//         //Generate random reviews data for the past 6 months
...
//                     count: Math.floor(Math.random() * 10), // Random reviews count between 0 and 9
//                 });
//             }
//             return reviews;
//         };
//         const reviewsData = generateRandomReviews();

//         reviewsData.forEach(review => {
//             const date = new Date(review.date);
//             const monthKey = `${months[date.getMonth()]}`;
//             if (monthlyData[monthKey]) {
//                 monthlyData[monthKey].reviews += review.count;
//             }
//         });
//         //Fetch PRs data from github for the past 6 months
//         const {data: prs} = await octokit.rest.search.issuesAndPullRequests({
//             q: `type:pr author:${user.login} created:>${sixMonthsAgo.toISOString().split("T")[0]}`,
//             per_page: 100,
//         });
